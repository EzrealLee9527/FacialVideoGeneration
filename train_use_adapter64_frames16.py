...
# from animatediff.data.webvid import WebVid
# from animatediff.data.self_collect_smile import SmileGif
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    dataset_cls = getattr(importlib.import_module('animatediff.data.dataset_new', package=None), data_class)
    ...
    # adapter module
    adapter_module_state_dict = torch.load(adapter_model_path, map_location="cpu")
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                prompt_ids = tokenizer(
                    [item for sublist in batch['texts'] for item in sublist], max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
            ...
            if is_main_process and global_step % checkpointing_steps == 0:
                ...
                if step == len(train_dataloader) - 1:
                    ...
                    torch.save(state_dict, os.path.join(save_path, f"checkpoint-epoch0-steps{checkpointing_steps}.ckpt"))
