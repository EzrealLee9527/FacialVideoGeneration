    ...
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    for model_idx, (config_key, model_config) in enumerate(list(config.items())):
        ...
        for motion_module in motion_modules:
            ...
            # image = tform(image) # 这里使用了裁剪
            
            # 加载gif
            import imageio
            import numpy as np
            reader = imageio.mimread(args.image_path)[:args.L]
            images = []
            for image in reader:
                image = tform(Image.fromarray(image[:,:,:3]))
                images.append(np.array(image))
            images = np.array(images)
            print('gif shape is', images.shape)
            images =  torch.Tensor((2.0 * images - 1.0)).cuda()
            ...
            for prompt_idx, (prompt, n_prompt, random_seed) in enumerate(zip(prompts, n_prompts, random_seeds)):
                ...
                video_length = min(len(reader), args.L)
                sample = pipeline(
                    prompt,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--image_path",            type=str, default="/work00/AnimateDiff-main/datas/nhsa.gif")
