...
def get_adapter_input(
    ...
    for frame_idx in range(start_idx, start_idx + frame_stride * video_length, frame_stride):
        if deca_replace == False:
            org_img, con_1, con_2, depth = read_vis(os.path.join(val_data_prefix, str(video_id), "deca", str(frame_idx), "vis.jpg"), resolution_w)
        else:
            org_img, _, _, _ = read_vis(os.path.join(val_data_prefix, str(video_id), "deca", str(frame_idx), "vis.jpg"), resolution_w)
            con_1 = read_vis_single(os.path.join(val_data_prefix, str(video_id), "deca_replace", str(frame_idx), "shape_images.jpg"), resolution_w)
            con_2 = read_vis_single(os.path.join(val_data_prefix, str(video_id), "deca_replace", str(frame_idx), "shape_detail_images.jpg"), resolution_w)
            depth = read_vis_single(os.path.join(val_data_prefix, str(video_id), "deca_replace", str(frame_idx), "depth.jpg"), resolution_w)
