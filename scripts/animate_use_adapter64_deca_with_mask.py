...
from animatediff.pipelines.pipeline_animation_inpainting import AnimationPipeline
...
def get_adapter_input(
    ...
    for frame_idx in range(start_idx, start_idx + frame_stride * video_length, frame_stride):
        if deca_replace == False:
            org_img, con_1, con_2, depth = read_vis(os.path.join(val_data_prefix, str(video_id), "deca_v231019", str(frame_idx), "vis.jpg"), resolution_w)
        else:
            org_img, _, _, _ = read_vis(os.path.join(val_data_prefix, str(video_id), "deca_v231019", str(frame_idx), "vis.jpg"), resolution_w)
            con_1 = read_vis_single(os.path.join(val_data_prefix, str(video_id), "deca_v231019", str(frame_idx), "shape_images.jpg"), resolution_w)
            con_2 = read_vis_single(os.path.join(val_data_prefix, str(video_id), "deca_v231019", str(frame_idx), "shape_detail_images.jpg"), resolution_w)
            depth = read_vis_single(os.path.join(val_data_prefix, str(video_id), "deca_v231019", str(frame_idx), "depth.jpg"), resolution_w)
        
        mask_img=mask_transform(Image.open(os.path.join(val_data_prefix, str(video_id), "deca", str(frame_idx), "{}.png".format(mask)))).mean(dim=0,keepdim=True) #1,H,W
        mask_list.append(mask_img)#N,H,W

        org_img_list.append(org_img)  
        con_1_list.append(con_1)
        con_2_list.append(con_2)
        depth_list.append(depth)
    mask_imgs=torch.stack(mask_list,dim=0)
    ...
    return adapter_input,mask_imgs,org_imgs/2.0+0.5
    ...
    for model_idx, (config_key, model_config) in enumerate(list(config.items())):
        ...
        for motion_module in motion_modules:
            ...
            if adapter_module != "":
                ...
                adapter_input,mask_imgs,org_imgs = get_adapter_input(
                ...
                adapter_input=adapter_input.cuda()
                mask_imgs=mask_imgs.cuda()
                org_imgs=org_imgs.cuda()
