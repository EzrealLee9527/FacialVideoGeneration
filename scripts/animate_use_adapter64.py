...
def load_tos_model(net_path, map_location):
    with smart_open(net_path, 'rb') as f:
        buffer = io.BytesIO(f.read())
        net = torch.load(buffer, map_location=map_location)
    return net
...
def main(args):
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    os.makedirs(savedir)
    # inference_config = OmegaConf.load(args.inference_config)   # NOTE

    config  = OmegaConf.load(args.config)
    samples = []
    samples_with_landmarks = []
    ...
    for model_idx, (config_key, model_config) in enumerate(list(config.items())):
        ...
        for motion_module in motion_modules:
            ...
            # 1. unet ckpt
            # 1.1 motion module
            motion_module_state_dict = load_tos_model(motion_module, map_location="cpu")
            ...
            if adapter_module != "":
                adapter_module_state_dict = load_tos_model(adapter_module, map_location="cpu")
                ...
                ldmks_video = model_config.landmark_video
                video_length = model_config.video_length       
                ...
                frames_ldmks = torch.tensor(frames_ldmks).permute(0,3,1,2).float().to("cuda")[:,:1,:,:]
                print('frames_ldmks2', frames_ldmks.shape)
            ...
            if model_config.path != "":
                if model_config.path.endswith(".ckpt"):
                    state_dict = load_tos_model(model_config.path)
            ...
            for prompt_idx, (prompt, n_prompt, random_seed) in enumerate(zip(prompts, n_prompts, random_seeds)):
                ...
                samples_with_landmark = torch.concat([save_frames_ldmks,sample],dim=-1)
                prompt = "-".join((prompt.replace("/", "").split(" ")[:10]))
                save_videos_grid(samples_with_landmark, f"{savedir}/sample/{sample_idx}-{prompt}.gif")
