...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    dataset_cls = getattr(importlib.import_module('animatediff.data.datasets_deca', package=None), data_class)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if epoch % 10 == 0 and step == 0:  # 每 10 个 epoch 进行 sanity check 一下
                pixel_values = batch['pixel_values'].cpu()
                con1_imgs = batch['con1_imgs'].cpu()
                con2_imgs = batch['con2_imgs'].cpu()
                depth_imgs = batch['depth_imgs'].cpu()
                if not image_finetune:
                    ...
                    for idx, (pixel_value, con1_img, con2_img, depth_img, text) in enumerate(zip(pixel_values, con1_imgs, con2_imgs, depth_imgs, texts)):
                        pixel_value = pixel_value / 2. + 0.5
                        # NOTE 这里不需要再乘以 255
                        torchvision.utils.save_image(pixel_value, f"{output_dir}/sanity_check/{idx}-org-{'-'.join(text[0].replace('/', '').split()[:10]) if not text == '' else f'{global_rank}-{idx}'}.png")
                        torchvision.utils.save_image(con1_img, f"{output_dir}/sanity_check/{idx}-con1-{'-'.join(text[0].replace('/', '').split()[:10]) if not text == '' else f'{global_rank}-{idx}'}.png")
                        torchvision.utils.save_image(con2_img, f"{output_dir}/sanity_check/{idx}-con2-{'-'.join(text[0].replace('/', '').split()[:10]) if not text == '' else f'{global_rank}-{idx}'}.png")
                        torchvision.utils.save_image(depth_img, f"{output_dir}/sanity_check/{idx}-depth-{'-'.join(text[0].replace('/', '').split()[:10]) if not text == '' else f'{global_rank}-{idx}'}.png")
            ...
            pixel_values = batch["pixel_values"].to(local_rank) # (b,c,h,w)
            ...
            # print('debug: ', len(texts), batch['pixel_values'].shape, batch['con1_imgs'].shape, batch['con2_imgs'].shape, batch['depth_imgs'].shape)
            if len(texts) == 0:
            ...
            cons_list = []
            if adapter_model.con1_wo_texture == True:
                cons_list.append(batch['con1_imgs'])
            if adapter_model.con2_texture == True:
                cons_list.append(batch['con2_imgs'])
            if adapter_model.con3_depth == True:
                cons_list.append(batch['depth_imgs'])
            ...
            assert 64 * adapter_input.shape[1] == adapter_model.cin
            # if image_finetune:
            ...
            with torch.cuda.amp.autocast(enabled=mixed_precision_training):
                ...
                # print("unet: ", unet.device)
                # print("noisy_latents: ", noisy_latents.device)
                # print("timesteps: ", timesteps.device)
                # print("encoder_hidden_states", encoder_hidden_states.device)
                # print("adapter_features", adapter_features.device)
            ...
            if is_main_process and global_step % checkpointing_steps == 0:
                ...
                # logging.info(f"Saved state to {local_save_path} (global_step: {global_step})")
            ...
            if is_main_process and (global_step % validation_steps == 0 or global_step in validation_steps_tuple):
                ...
                img = Image.open(validation_data.vis_img_path).convert("RGB")

                w = 224
                resize_w = 256
                resize_transform = transforms.Resize((resize_w, resize_w))
                # org_img = resize_transform(img.crop((0,0,w,w)))
                con_1 = np.array(resize_transform(img.crop((3*w,0,4*w,w))))
                con_2 = np.array(resize_transform(img.crop((4*w,0,5*w,w))))
                depth = np.array(resize_transform(img.crop((5*w,0,6*w,w))))

                con1_imgs = torch.tensor(con_1).unsqueeze(0).permute(0, 3, 1, 2).float()
                con2_imgs = torch.tensor(con_2).unsqueeze(0).permute(0, 3, 1, 2).float()
                depth_imgs = torch.tensor(depth).unsqueeze(0).permute(0, 3, 1, 2).float()
