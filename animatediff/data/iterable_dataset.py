...
class S3IterableDataset(IterableDataset):
    def __init__(self, s3_bucket, s3_key, aws_access_key_id, aws_secret_access_key):
    ...
    def __iter__(self):
        ...
        # 从 S3 下载对象，并包装为 BytesIO 对象
        s3_object = s3_client.get_object(Bucket=self.s3_bucket, Key=self.s3_key)
        byte_stream = io.BytesIO(s3_object["Body"].read())

        # 在此处处理数据流并生成训练样本（这取决于您的数据格式和需求）

        # 示例：逐行读取文本文件，并将其视为字符串对待
        for line in byte_stream:
            yield line.decode("utf-8").strip()

class S3IterableVideosDataset(IterableDataset):   
    def __init__(self,
                 data_dir,
                 data_dir_prefix,
                 subsample=None,
    ...
    def _sample_generator(self, start, end):
        for _ in range(start, end):
            f_idx = np.random.randint(0, len(self.meta_info_list))
            try:
                ...
                print('landmarks', landmarks[0].shape)
                ...
                print(frames_landmarks[0].shape)
                print(type(frames_landmarks[0]))
                ...
                frames_imgs = F.interpolate(input=frames_imgs[...,crop_y1:crop_y2, crop_x1:crop_x2], size=self.resolution, mode='bilinear', align_corners=False)
...
if __name__ == '__main__':
    data_dir_prefix = '/work00/AnimateDiff-adapter-mmv2/animatediff/data'
    data_dir = '/work00/AnimateDiff-adapter-mmv2/animatediff/data/videos1600_gen'
    frame_stride = 1
    is_image = True
    dataset = S3IterableVideosDataset(data_dir, data_dir_prefix, frame_stride=frame_stride, is_image=is_image)
    ...
    for idx, batch in enumerate(dataloader):
        ...
        if is_image:
            ...
            with imageio.get_writer(f"debug/{idx}_{caption}_video_all.mp4", fps=30) as video_writer:
