...
def load_faceparsing_model(device): # NOTE
    # 建立模型
    global face_detector, face_parser
    face_detector = facer.face_detector('retinaface/mobilenet', device=device)
    face_parser = facer.face_parser('farl/lapa/448',model_name='/data00/fsq/FaRL-main/facer_use/face_parsing.farl.lapa.main_ema_136500_jit191.pt', device=device) # optional "farl/celebm/448"
    return face_detector, face_parser

def gen_faceparsings(fp_img, label_path, vis_path):

    try: 
        fp_img = rearrange(fp_img, "h w c -> c h w").unsqueeze(0)
        with torch.inference_mode():
            fp_faces = face_detector(fp_img)  
        
        with torch.inference_mode():
            fp_faces = face_parser(fp_img, fp_faces)

        seg_logits = fp_faces['seg']['logits']    # torch.Size([1,11,h,w])
        seg_probs = seg_logits.softmax(dim=1)  # nfaces x nclasses x h x w  
        n_classes = seg_probs.size(1)
        vis_seg_probs = seg_probs.argmax(dim=1).float()/n_classes*255  # [1,512,512], 在类别这个维度上取了最大的index, 并以此为依据平均划分颜色
        vis_img = vis_seg_probs.sum(0, keepdim=True)                  

        fp_images, seg_labels = facer.draw_bchw(fp_img, fp_faces)
        # seg_labels[0][0].shape: (h, w)

        print("faces num: ", len(seg_labels[0]))
        
        all_faces_label = np.zeros([fp_img.shape[2], fp_img.shape[3]])
        for one_face_label in seg_labels[0]:
            all_faces_label = all_faces_label + one_face_label
        # 将叠加起来大于 10 的地方都置为 0
        if np.max(all_faces_label) > 10:
            print("Warning: > 10 !!!")
        # all_faces_label = np.where(all_faces_label > 10.0, 0, all_faces_label)

        # 保存 label 标签为灰度图
        # label_image = Image.fromarray(all_faces_label, mode='L')  # 调试
        # label_image = Image.fromarray(all_faces_label, mode='L')
        # label_image.save(os.path.join(vis_result_save_path, "label_{}.png".format(str(frame_idx))))
        # 保存 label 标签为 numpy 数组
        # np.save(os.path.join(vis_result_save_path, "label_{}.npy".format(str(frame_idx))), all_faces_label)
        # 保存 label 标签为 RGB 图
        all_faces_label = np.repeat(all_faces_label[:,:,np.newaxis], 3, axis=2)
        all_faces_label = np.uint8(all_faces_label)
        dump_img(all_faces_label, label_path)

        vis_result = rearrange(fp_images.squeeze(0), "c h w -> h w c")  # 把 batch 维度除掉
        vis_result = np.uint8(vis_result.cpu())
        dump_img(vis_result, vis_path)
        # vis_result: (h, w, 3)

        print("success:  {}".format(label_path))

        return "success"

    except Exception as e:
        print("fail:  {}".format(label_path), e)
        return "fail"
...
def worker(args):
    ...
    # TODO:
    # face_detector, face_parser = load_faceparsing_model(device=device) # NOTE
    ...
    for res in videos_gen:
        ...
        for batch_frame_idxs, batch_frames in zip(
            ...
            for k in range(len(batch_frames)):
                ...
                # TODO
                # face parsings
                # batch_frames[k]: (h,w,3)，0-255, h, w 是没有经过 resize 过的
                fp_img = batch_frames[k].to(device)
                label_f = os.path.join(
                    condition_out_dir, video_fname, 'labels', f'{frame_idx}.png')
                vis_f = os.path.join(
                    condition_out_dir, video_fname, 'vis', f'{frame_idx}.png')
                gen_faceparsings(fp_img, label_f, vis_f)
                       

                img_f = os.path.join(
                    condition_out_dir, video_fname, 'frames', f'{frame_idx}.png')
                dump_img(batch_frames[k].numpy(), img_f) # 这个函数用于储存图片
