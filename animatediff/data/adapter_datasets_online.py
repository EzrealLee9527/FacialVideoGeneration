    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            try:
                ...
                # input [t,h,w,c]
                if self.is_image:
                    landmarks, _ = get_landmarks(frames[0:1])
                else:
                    landmarks, _ = get_landmarks(frames)    
                # process face parsing

                frames = torch.tensor(frames).permute(0, 3, 1, 2).float()  # [t,h,w,c] -> [t,c,h,w]
                landmarks = torch.tensor(np.array(landmarks)).unsqueeze(-1).permute(0, 3, 1, 2).float() 
...
if __name__ == '__main__':
    ...
    meta_path = mount_path + '/Videos/CelebV-Text/descripitions/filelist.txt'
    ...
    for idx, batch in enumerate(dataloader):
        ...
        if is_image:
            ...
            with imageio.get_writer(f"debug/{caption}_video.mp4", fps=30) as video_writer:
