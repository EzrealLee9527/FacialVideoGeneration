...
class SmileGif(Dataset):
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            try:                
                if self.load_raw_resolution:
                    ...
                    f_idx = np.random.randint(0,len(reader))
                    frame = reader[f_idx]
        ...
        frame = torch.tensor(np.array(resized_frame)).permute(
                    2, 0, 1).float()[:3,:,:]  # [h,w,c] -> [c,h,w]
        frame_ldmks = torch.tensor(np.array(frame_ldmks)).permute(
                    2, 0, 1).float()
        if self.spatial_transform is not None:
            frames = self.spatial_transform(frames)
        ...
        # if self.is_image:
        #     frames = frames[0]
        sample = dict(pixel_values=frame, landmarks=frame_ldmks, text=caption, rel_fp=rel_fp)
        return sample
...
if __name__ == '__main__':
    ...
    for idx, batch in enumerate(dataloader):
        ...
        print(batch["pixel_values"].shape, batch["landmarks"].shape,batch["text"])
        print(batch["pixel_values"].max(), batch["pixel_values"].min())
        rel_fp= batch['rel_fp']
        video_array = ((batch["pixel_values"].permute(0, 2, 3, 1)+1)/2 * 255).numpy().astype(np.uint8)
        landmarks = ((batch["landmarks"].permute(0, 2, 3, 1)+1)/2 * 255).numpy().astype(np.uint8)

        caption = batch["text"][0]
        for i in range(video_array.shape[0]):
            cv2.imwrite(f"{idx}_{i}_{rel_fp[0]}_{caption}_img.png", video_array[i])
            cv2.imwrite(f"{idx}_{i}_{rel_fp[0]}_{caption}_ldmks.png", landmarks[i])
