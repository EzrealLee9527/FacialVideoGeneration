...
def load_model():
    if args.backbone=='MobileNet':
        ...
        checkpoint = torch.load('pytorch_face_landmark/checkpoint/mobilenet_224_model_best_gdconv_external.pth.tar', map_location=map_location)
        ...
        checkpoint = torch.load('pytorch_face_landmark/checkpoint/pfld_model_best.pth.tar', map_location=map_location)
...
if __name__ == '__main__':
    ...
    model = model.eval()
    model = model.cuda()
    filenames=glob.glob("/work00/AnimateDiff-adapter-mmv2/animatediff/data/out2/worker0/conditions/4607440/frames/*.png")
    # filenames=glob.glob("pytorch_face_landmark/samples/12--Group/*.jpg")
    ...
    for imgname in filenames:
        ...
        if args.detector=='MTCNN':
            ...
            print('MTCNN Time: {:.6f}s.'.format(end - start))
            print('landmarks', len(landmarks), len(landmarks[0]))
        elif args.detector=='FaceBoxes':
            face_boxes = FaceBoxes()
            faces = face_boxes(img)
        elif args.detector=='Retinaface':
            start = time.time()
            retinaface=Retinaface.Retinaface('cuda')    
            faces = retinaface(img)      
            end = time.time()
            print('Retinaface Time: {:.6f}s.'.format(end - start))      
        else:
