...
from animatediff.data.datasets_deca import read_vis   # NOTE: 注意 datasets 使用哪一个文件
...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    dataset_cls = getattr(importlib.import_module('animatediff.data.datasets_deca', package=None), data_class) # NOTE：注意 datasets 使用哪一个文件
    ...
    # adapter module
    print('load trained adapter model')
    if adapter_model.in_tos == True:
        with smart_open(adapter_model.adapter_model_path, 'rb') as f:
            buffer = io.BytesIO(f.read())
            adapter_module_state_dict = torch.load(buffer, map_location="cpu")
    else:
        adapter_module_state_dict = torch.load(adapter_model.adapter_model_path, map_location="cpu")
    ...
    model_adapter = Adapter(cin=adapter_model.cin, channels=[320, 640, 1280, 1280][:4], nums_rb=2, ksize=1, sk=True, use_conv=False).to("cuda")
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            pixel_values = batch["pixel_values"].to(local_rank)  # pixel_values: [b,f,c,h,w]
            with torch.no_grad():
                if not image_finetune:
            ...
            if adapter_model.con3_depth == True:
                cons_list.append(batch['depth_imgs']) # [b,f,c,h,w]
            assert len(cons_list) > 0

            adapter_input = torch.cat(cons_list, dim=2)  
            adapter_input = rearrange(adapter_input, "b f c h w -> (b f) c h w")
            ...
            if is_main_process and (global_step % validation_steps == 0 or global_step in validation_steps_tuple):
                ...
                for frame_idx in range(validation_data.start_idx, validation_data.start_idx + validation_data.frame_stride * video_length, validation_data.frame_stride):
                    org_img, con_1, con_2, depth = read_vis(os.path.join(validation_data.val_data_prefix, str(frame_idx), "vis.jpg"), train_data.resolution[1])
