...
class TestData(Dataset):
    def __init__(self, testpath, iscrop=True, crop_size=512, scale=1.25, face_detector='fan', sample_step=10):
        '''
        ...
        for s3_dir in s3_dirs:
            imagepath_list_one = glob(s3_dir + '/frames/*.jpg') +  glob(s3_dir + '/frames/*.png') + glob(s3_dir + '/frames/*.bmp')
            self.imagepath_list.extend(imagepath_list_one) 
    ...
    def __getitem__(self, index):
        ...
        image = np.array(imread(open(imagepath,'rb')))
...
class LoraData(Dataset):
    def __init__(self, testpath, iscrop=True, crop_size=512, scale=1.25, face_detector='fan', sample_step=10):
        ...
        self.imagepath_list = []
        image_path = glob(testpath+'/*.jpg') + glob(testpath+'/*.jpeg') +  glob(testpath + '/frames/*.png') + glob(testpath+ '/frames/*.bmp')
        self.imagepath_list.extend(image_path)
        # for s3_dir in s3_dirs:
        #     imagepath_list_one = glob(s3_dir + '/frames/*.jpg') +  glob(s3_dir + '/frames/*.png') + glob(s3_dir + '/frames/*.bmp')
        #     self.imagepath_list.extend(imagepath_list_one) 
        print('total {} images'.format(len(self.imagepath_list)))
